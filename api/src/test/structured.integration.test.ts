...
describe('Structured Data Extraction API', () => {
  ...
  test.each([
    ["groq", "https://api.groq.com/openai/v1", "401 Invalid API Key"],
    ["openrouter", "https://openrouter.ai/api/v1", "401 No auth credentials found"],
    ["openai", "https://api.openai.com/v1", "401 Incorrect API key provided: INVALID. You can find your API key at https://platform.openai.com/account/api-keys."]
  ])('rejects invalid API key for %s provider', async (provider, url, expectedMessage) => {
    const input = "abc123";
    const schema = {
      type: "object",
      properties: {
    ...
    expect(result.message).toBe("Failed to call provider");
    expect(result.providerMessage).toBe(expectedMessage);
  });
  ...
  test('rejects non-compliant schema', async () => {
    ...
    expect(response.status).toBe(400);
    expect(result.message).toBe("Disallowed property 'minLength' found in schema");
  });
